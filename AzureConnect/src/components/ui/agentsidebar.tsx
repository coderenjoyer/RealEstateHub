...
export function Sidebar() {
  ...
  const isActive = (itemTo: string) => {
    ...
    return (
      pathname === itemTo ||
      (pathname.startsWith(itemTo + "/") && pathname !== itemTo + "/")
    );
  };

  return (
    <>
      {/* Mobile Toggle Tab - Sticks out from left edge */}
      {isMobileView && (
        <button
          onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)}
          className={cn(
            "fixed top-1/2 -translate-y-1/2 z-50 transition-all duration-300",
            "w-8 h-20 bg-gradient-to-r from-[#0A4174] to-[#49769F] hover:from-[#0A4174]/90 hover:to-[#49769F]/90",
            "text-white rounded-r-2xl shadow-lg hover:shadow-xl",
            "flex items-center justify-center",
            "focus:outline-none focus:ring-2 focus:ring-[#0A4174]",
            isMobileMenuOpen
              ? "left-64"
              : "left-0"
          )}
      ...
      <aside
        className={cn(
          ...
          "md:translate-x-0",
          isMobileView && !isMobileMenuOpen && "-translate-x-full",
          isMobileView ? "w-64" : isCollapsed ? "w-16" : "w-64"
        )}
      ...
      >
        ...
        <div
          className={cn(
            ...
            isCollapsed && !isMobileView ? "justify-center px-4" : "px-6"
          )}
        >
          ...
          >
            <h1 className="text-lg font-bold text-[#0A4174] whitespace-nowrap">
              RealEstate
            </h1>
            <p className="text-xs text-[#49769F] whitespace-nowrap">
              Agent Portal
            </p>
          </div>
        </div>
            ...
            return (
              ...
              >
                <Link
                  ...
                  onClick={(e) => {
                    // Prevent any bubbling that might trigger expansion
                    e.stopPropagation();
                  }}
                  className={cn(
                ...
                >
                  ...
                  {/* Hover glow effect */}
                  {!active && (
                    <div className="absolute inset-0 bg-gradient-to-r from-[#0A4174]/0 via-[#0A4174]/15 to-[#49769F]/0 opacity-0 group-hover:opacity-100 transition-opacity rounded-xl" />
                  )}
                </Link>

                {/* Tooltip for collapsed state */}
                {isCollapsed && !isMobileView && hoveredItem === index && (
                  <div
                    className="absolute left-full top-1/2 -translate-y-1/2 ml-4 px-4 py-2 bg-white text-[#0A4174] text-sm rounded-lg whitespace-nowrap z-50 pointer-events-none shadow-xl border border-[#49769F]"
                    role="tooltip"
                  >
                    {item.label}
                    <div className="absolute right-full top-1/2 -translate-y-1/2 -mr-1 border-8 border-transparent border-r-white" />
                  </div>
                )}
              </div>
            );
        ...
        <div className="border-t border-[#49769F] p-3">
          <button
            ...
            className={cn(
              ...
              "hover:bg-white/60 hover:text-[#0A4174] transition-all duration-200 group",
              "focus:outline-none focus:ring-2 focus:ring-[#0A4174] focus:ring-offset-2 focus:ring-offset-[#BDD8E9]",
              isCollapsed && !isMobileView ? "justify-center px-2" : "px-4"
            )}
          >
            <LogOut
      ...
      {!isMobileView && (
        <button
          ...
          className={cn(
            ...
            isCollapsed
              ? "left-16 -translate-x-5 hover:translate-x-0"
              : "left-64 -translate-x-5 hover:translate-x-0"
          )}
        >
          <ChevronLeft
            className={cn(
